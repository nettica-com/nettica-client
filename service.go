...
func (m *Nettica) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (ssec bool, errno uint32) {
	...
	elog.Info(1, "Nettica Service Started")
	DoWork()
...
func makeMesh(configPath string) error {
	...
	exepath = strings.Replace(exepath, "nettica-client.exe", "wireguard.exe", -1)
	...
	desc := fmt.Sprintf("Network: %s", name)
...
func removeMesh(name string) error {
	...
	s, err := m.OpenService(serviceName)
	if err != nil {
		return fmt.Errorf("net  %s is not installed", name)
	}
	defer s.Close()
	err = s.Delete()
	if err != nil {
...
func usage(errmsg string) {
	fmt.Fprintf(os.Stderr,
		"%s\n\n"+
			...
			"       install, remove, makenet, removenet, debug, start, stop, pause or continue.\n",
		errmsg, os.Args[0])
	os.Exit(2)
}
