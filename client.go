...
func (w *Worker) CallNettica(etag *string) ([]byte, error) {
	...
	if err != nil {
		// Disappointing to have to add this "workaround" for Docker losing DNS settings "randomly"
		// but we can't have the service going down just because of a DNS failure.
		log.Errorf("DNS lookup for %s failed: %v", host, err)
		if ServiceHost && runtime.GOOS == "linux" {
			err := os.WriteFile("/etc/resolv.conf", []byte("# Generated by resolvconf\nnameserver 1.1.1.1\nnameserver 8.8.8.8\n"), 0644)
			if err != nil {
				log.Error(err)
			}
		}
